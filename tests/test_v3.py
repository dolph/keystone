...
class RestfulTestCase(test_content_types.RestfulTestCase):
    ...
    def new_policy_ref(self, endpoint_id):
        ref = self.new_ref()
        ref['blob'] = uuid.uuid4().hex
        ref['type'] = uuid.uuid4().hex
        ref['endpoint_id'] = endpoint_id
        return ref
    ...
    def setUp(self):
        self.config([
            ...
            # test.testsdir('backend_kvs.conf')])
    ...
    def assertValidEntity(self, entity, ref=None):
        ...
        self.assertIsNotNone(entity.get('enabled'))
        ...
        if ref:
            self.assertEquals(ref['name'], entity['name'])
            self.assertEquals(ref['description'], entity['description'])
            self.assertEquals(ref['enabled'], entity['enabled'])
